...
public:
    ...
    void initHandlers(config::AllCommands cmnds)
    {
        ...
        reg_.registrateCommand(cmnds.stop_all.command, &handlers::CommandsProcessor::stopAll, &PermissionChecker::checkBaseCommand, {0, 0});
        // Задачи
        ...
        // Сообщения
        reg_.registrateMessageInChat(&handlers::MessagesProcessor::addMessage, &PermissionChecker::checkMessage);
        reg_.registrateMessageAddress(&handlers::MessagesProcessor::processMessage, &PermissionChecker::checkMessage);
    }
